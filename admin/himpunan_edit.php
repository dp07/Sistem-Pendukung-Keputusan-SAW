...
<!-- Body -->
		...
		<!-- Page title -->
			...
			$query=mysqli_query($konek, "SELECT * FROM himpunan WHERE id_himpunan='$id_himpunan'");
			...
			if (isset($_POST['ubah'])) {
				...
				$query=mysqli_query($konek, "UPDATE himpunan SET id_kriteria='$id_kriteria', namahimpunan='$namahimpunan', nilai='$nilai', keterangan='$keterangan' WHERE id_himpunan='$id_himpunan'") or die(mysqli_error($konek));
				if ($query) {
				echo "<script>
						document.location.href='himpunan.php';
					 </script>";
				}}
			?>
				<!-- /right labels -->
